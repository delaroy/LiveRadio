...
public class Track extends MusicEntry {
	 ...
	 * @return list of tags
	 */
	public static List<Tag> getTopTags(String artist, String trackOrMbid,
			String apiKey) {
		Map<String, String> params = new HashMap<String, String>();
	...
	public static Track getInfo(String artist, String trackOrMbid, String apiKey) {
		...
		return trackFromElement(element);
	}
	...
	static Track trackFromElement(DomElement element, String artistName) {
		...
		if (element.hasChild("artist")) {
			track.artist = element.getChild("artist").getChildText("name");
			if (track.artist == null)
				track.artist = element.getChildText("artist");
		}

		if (element.hasChild("album")) {
			track.album = element.getChild("album").getChildText("title");
			if (track.album == null)
				track.album = element.getChildText("album");
		}

		if (element.hasChild("toptags")) {
			for (DomElement o : element.getChild("toptags").getChildren("tag")) {
				track.tags.add(o.getChildText("name"));
			}
		}
		return track;
	}
